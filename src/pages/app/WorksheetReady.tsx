...
import { useStudyContext, contextDescription } from "@/prototype/context";
...
export default function WorksheetReady() {
  ...
  return (
    <AppLayout>
      <BackToParent to={link("/app/worksheets")} label="Back to Worksheets" />
      <PageHeader
        kicker="Level 3 improvement · Worksheet loop"
        title="Your worksheet is ready"
        sub={`${ctx.subject} · ${ctxDesc || "Full subject"} · 30 marks · Sections A–E`}
      />
      ...
      <div className="lt-card p-6 max-w-3xl">
        ...
        <div className="flex flex-wrap gap-2">
          <Link to={link("/app/worksheets/attempt")}><Button className="bg-accent hover:bg-accent/90 text-accent-foreground">Attempt on screen →</Button></Link>
          <Button variant="outline">Download PDF</Button>
          <Link to={link("/app/check", { source: "worksheet" })}><Button variant="ghost">Check an answer</Button></Link>
        </div>
      </div>
    </AppLayout>
  );
